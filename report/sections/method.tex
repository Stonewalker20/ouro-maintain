...
The evaluation compares the adaptive looped model against four baselines: a direct GRU-based classifier, a fixed-depth version of the same looped architecture, a generic pretrained text-transformer baseline, and a task-specific text-transformer baseline. Both LLM variants serialize each telemetry window into a compact textual summary. The generic pretrained baseline applies a DistilBERT encoder with lightweight multitask heads, while the task-specific baseline unfreezes the top DistilBERT encoder block and fine-tunes it directly for maintenance supervision. Together, these comparisons isolate the value of recurrence, adaptive early exit, and numeric telemetry modeling versus increasingly task-specialized text-transformer inference.
