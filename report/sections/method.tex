...
The current implementation uses a GRU encoder with hidden size 128, AdamW optimization, and a maximum loop depth of six steps. The adaptive model halts when the classifier confidence exceeds 0.8. The pipeline records average depth per example, validation and test confusion matrices, and full metric reports for each run.
