...
Reproducibility depends on documenting the exact dataset version, preprocessing parameters, random seeds, and train/validation/test split definitions. That matters especially for IMS, where label construction is derived and therefore sensitive to the chosen failure horizon. The artifact scaffold records the command-line configuration, task-specific metrics, and model checkpoints for each variant.
